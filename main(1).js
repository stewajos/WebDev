...
function getUsers(req, res){
	user = req.id 
    var user = {"id" : 1010, "name": "Matt"}
    res.status(200).send(user);
}
function getJournals(req, res){
    var user = {"id" : 1010, "name": "Jeff"}
    res.status(200).send(user);
}
function getImages(req, res){
    var user = {"id" : 1010, "name": "Timothy"}
    res.status(200).send(user);
}
function getComments(req, res){
    var user = {"id" : 1010, "name": "Allen"}
    res.status(200).send(user);
}
function getLocations(req, res){
    var user = {"id" : 1010, "name": "Jimmy"}
    res.status(200).send(user);
}
function getStatic(req, res){
    var user = {"id" : 1010, "name": "Shaun"}
    res.status(200).send(user);
}
